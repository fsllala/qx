...
2023/10/13
重写：
https://ikuuu.art/user/checkin url script-request-body https://raw.githubusercontent.com/fsllala/qx/main/ikuuu.js
主机名：
ikuuu.art
*/
const $ = new Env("ikuuu");
const names = "ikuuuSing";
const zh_name = "爱坤签到";

const getxxxHeaders = $.getdata(`${names}header`);
const getxxxBody = $.getdata(`${names}body`);
...
async function GetCookie() {
  let reg = /.*ikuuu.art\/user\/checkin$/;
  if (reg.test($request.url)) {
    const reqHeaders = JSON.stringify($request.headers);
    if (reqHeaders) $.setdata(reqHeaders, `${names}header`);
    $.log(reqHeaders);

    const reqBody = $request.body;
    if (reqBody) $.setdata(reqBody, `${names}body`);
    $.log(reqBody);

    $.msg($.names, "", `${zh_name}获取ck成功`);
  } else {
...
async function checkin() {
  const singUrl = "https://ikuuu.art/user/checkin";
  const res = await $.http.post({
    url: singUrl,
    headers: getxxxHeaders,
  });
  const message = JSON.stringify(decodeURI(res.msg));
  if (res.ret == 1) {
    $.msg(zh_name, "success", message);
  } else {
    $.msg(zh_name, "error", message);
  }
}
