...
*/
const $ = new Env("wzdy");
const names = "wzdySing";
const zh_name = "王者营地签到";
...
async function GetCookie() {
  let reg = /.*kohcamp.qq.com\/operation\/action\/signin/;
...
async function checkin() {
  const singUrl = "https://kohcamp.qq.com/operation/action/signin";
  ...
  if (res.returnCode == 0) {
    const message = JSON.stringify(res.data.userTotalSign);
    $.msg(zh_name, "success", `本月共签到${message}天`);
  } else {
    $.msg(zh_name, "error", res.returnMsg);
  }
}
